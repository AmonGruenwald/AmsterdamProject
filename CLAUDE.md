...
| `src/wallen.js` | De Wallen inhabitants: window dancers at every front (three styles, twirls), heart kisses, charm streaks, rose petals, the heart bike-trail, water shimmer, `WallenMuziek` (bass + heat-driven sax lead; init needs a user gesture — main.js calls it from the start button; `pulse()` drives the beat-synced neon throb), the `Q` dance-off with encore levels (`danceLevel` 0–3 scales beat/length/jackpot; base tempo = `DANCE_BEAT`), and the Verleiding/sweetheart progression (`verleiding` 0–100 → `onSweetheart`, then `onGift` perks). Keep everything silhouette-and-wink. |
